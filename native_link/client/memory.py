...
class MemoryLayer:
    ...
    def __init__(self):
        ...
        # use settings to populate memory llm config if missing
        s = cfg.get("settings", {})

        # use url connection to configured host/port
        self.qdrant = QdrantClient(
            url=s.get("qdrant_url", "http://127.0.0.1:6334"),
            prefer_grpc=True,
            check_compatibility=False
        )
        
        mem_cfg = cfg.get("memory", {})
        mem_cfg.setdefault("llm", {
            "provider": "openai",
            "config": {
                "model": s.get("model"),
                "temperature": s.get("temperature"),
                "openai_base_url": s.get("base_url"),
                "api_key": s.get("api_key")
            }
        })
        mem_cfg.setdefault("vector_store", {"provider": "qdrant", "config": {"client": self.qdrant}})
        
        # baai/bge-small-en-v1.5 has a dimension of 384
        self.dims = mem_cfg.get("embedder", {}).get("config", {}).get("embedding_dims", 384)
        
        # ensure collections exist with correct dimensions BEFORE mem0 initialization
        ...
        self.mem0 = Memory.from_config(mem_cfg)
        self.max_sessions = s.get("max_sessions", 10)

        if MemoryLayer._embedder is None:
            MemoryLayer._embedder = SentenceTransformer(
                mem_cfg.get("embedder", {}).get("config", {}).get("model", "BAAI/bge-small-en-v1.5"), 
                trust_remote_code=True
            )
        self.embedder = MemoryLayer._embedder

        if MemoryLayer._reranker is None:
            reranker_cfg = mem_cfg.get("reranker", {})
        ...
        retrieval = mem_cfg.get("retrieval_settings", {})
