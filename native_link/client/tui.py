                ...
                try:
                    ...
                    if "command" in item:
                        ...
                        if self._active_prompt_widget:
                            ...
                            # if no active prompt, just add it as a new entry with the standard prompt
                            self._add_log_entry(Text.from_markup(self._prompt_text) + Text(cmd))

                    # handle output dictionary
                    if "output" in item:
                        out_text = str(item["output"])
                        # add a small gap before output if it's substantial
                        if len(out_text.strip()) > 0:
                            self._add_log_entry("") 
                        for line in out_text.splitlines():
                            self._add_log_entry(Text.from_ansi(line))
                        # add a small gap after output
                        if len(out_text.strip()) > 0:
                            self._add_log_entry("") 
                    ...
                    if "new_prompt" in item:
                        ...
                        # always show a new prompt after a command/output sequence
                        self._show_prompt()
...
class JsonEditor(Widget):
    ...
    FILES = [
        Path(__file__).parent / "config/machine_state.json",
        Path(__file__).parent / "config/archive.json"
    ]
...
class ConfigEditor(Widget):
    ...
    FILE = Path(__file__).parent / "config/sys_config.yaml"
